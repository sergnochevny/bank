  ...
  BEGIN
    ...
    DECLARE contracts_cur CURSOR FOR
      SELECT
        ...
        calculation_id
      FROM contracts_calculation_prms
      WHERE month_day = dayofmonth(from_unixtime(inOnDate)) AND last_month_day = FALSE;
...
/*!40000 ALTER TABLE `contract_features`
  DISABLE KEYS */;
/*!40000 ALTER TABLE `contract_features`
  ENABLE KEYS */;
...
CREATE ALGORITHM = UNDEFINED
  DEFINER =`root`@`%`
  SQL SECURITY DEFINER VIEW `contracts_calculation_prms` AS
  SELECT
    `a`.`id`              AS `account_id`,
  ...
  FROM ((((`contract_features` `cf`
    JOIN `contract` `c` ON ((`cf`.`contract_id` = `c`.`id`))) JOIN `account` `a`
      ON ((`cf`.`contract_id` = `a`.`contract_id`))) LEFT JOIN `conditions` `cnd`
      ON ((`cf`.`contract_id` = `cnd`.`contract_id`))) JOIN `calculation` `cc`
      ON ((`cnd`.`calculation_id` = `cc`.`id`)))
  WHERE (`cf`.`closed` = 0);
